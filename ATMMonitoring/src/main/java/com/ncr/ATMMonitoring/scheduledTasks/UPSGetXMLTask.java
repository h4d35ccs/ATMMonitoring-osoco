...
/**
 * Scheduled task that reads XML files with UPS data from the file system
...
public class UPSGetXMLTask {
	...
	/** Folder where we want to copy/move the processed files (only needed with 'move' and 'copy') */
	...
	/** Internal variable with the operation to do with processed files (delete by default) */
	private int copyDeleteMove = 0;

	/** Copy option: each successfully processed file will be copied to a folder keeping the original */
	private static final int COPY_SUCCESSFUL_FILE = 1;

	/** Delete option: each successfully processed file will be permanently deleted */
	private static final int DELETE_SUCCESSFUL_FILE = 0;

	/** Move option: each successfully processed file will be copied to a folder deleting the original */
	 ...
	 * 
	 * @param filesPaths list with paths to the files to process
	 */
	private void handleSuccess(List<String> filesPaths) {
		...
		case DELETE_SUCCESSFUL_FILE:
			...
			FileInDiskHandler
					.delete(filesPaths, FileInDiskHandler.IGNORES_ERROR);
			break;
