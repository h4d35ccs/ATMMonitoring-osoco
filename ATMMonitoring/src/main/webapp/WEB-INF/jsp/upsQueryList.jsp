...
<t:osoco-wrapper titleCode="label.upss" userMsg="${userMsg}" section="query">
  ...
  <script type="text/javascript">
    $(function() {
        $("tr.queryRow").click(function(event) {
            ...
            window.location.assignWithBase("upsQueries/show?queryId=" + queryId);
        });
        ...
        $(".delete").click(function(event) {
            ...
            event.preventDefault();
            var queryId= $(this).data('queryId');
            if (confirm('<spring:message code="label.query.confirmDeletion"/>')) {
                window.location.assignWithBase("upsQueries/delete?queryId=" + queryId);
            }
        });
    });
  </script>
...
<jsp:body>
  ...
  <div class="content">
    ...
    <div class="botonera">
      <a href="upsQueries/create" class="btn right"><spring:message code="label.query.new"/></a>
    </div>
	...
	<c:forEach items="${pagedListHolder.pageList}" var="query">
	  <tr class="queryRow" data-query-id="${query.id}">
		...
		<a href="upsQueries/show?queryId=${query.id}">${query.name}</a></div></td>
	...
	<div class="t_number"><span class="text">${pagedListHolder.source.size()} <spring:message code="label.upsQueries"/></span></div>
	<c:if test="${pagedListHolder.getPageCount() >1 }">
	  <div class="p_number"><span class="text"><spring:message code="label.page"/></span><t:paging pagedListHolder="${pagedListHolder}" pagedLink="upsQueries/list?p=~"/></div>
	</c:if>
      </div>
    </c:if>
    <c:if test="${empty pagedListHolder.pageList}">
